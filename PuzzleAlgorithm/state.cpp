...
{
	int deFactor = pow(10,N-1);
	int inFactor = 1;
	for(int i=0;deFactor!=0;deFactor/=10,inFactor*=10){
		vect[i] = id/deFactor%inFactor;
		i++;
	}
}

bool State::isGoodOrder()
{
...
void getBestWayToStack(int stateid,stack<State>& result)
{
	...
	it = allPathMap.find(stateid);
	if(it == allPathMap.end()){
		cerr<<"Sorry,the stateid doesn't exist"<<endl;
		return;
	}
	State *p = it->second;
	while(p!=NULL){
